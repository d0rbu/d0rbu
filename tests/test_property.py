...
from henry_castillo.content import Card, CardError, CardLinks, CardProfile

_str = st.text(max_size=40)

# Required-field strings: non-empty AND non-blank (parse_card uses _req which
# rejects "" but also _sanitize_strict strips nothing here — however a string
...
_req_str = _str.filter(lambda s: s != "")
        ...
        "profile": st.fixed_dictionaries(
            {
                ...
                "about": _req_str,
                "email": _req_str,
                "links": st.fixed_dictionaries({"github": _req_str, "blog": _str}),
            }
        ),
        "projects": st.lists(
        ...
        "resume": st.fixed_dictionaries(
            {
                "pdf": _str,
                "experience": st.lists(
                    st.dictionaries(st.text(max_size=8), _str, max_size=4),
                    max_size=3,
                ),
                "education": st.lists(
                    st.dictionaries(st.text(max_size=8), _str, max_size=4),
                ...
                "highlights": st.lists(_str, max_size=4),
            }
        ),
    }
)
...
def test_blog_url_invariant(blog):
    profile = CardProfile(
        ...
        about="a",
        email="e",
        links=CardLinks(github="g", blog=blog),
    )
